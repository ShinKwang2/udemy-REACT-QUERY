  ...
  return (
    <>
      ...
      <div>
        ...
        {updateMutation.isPending && <p className="loading">Update the post title</p>}
        {updateMutation.isError && (
          <p className="error">Error update the post title: {updateMutation.error.toString()}</p>
        )}
        {updateMutation.isSuccess && <p className="success">Post title was (not) updated</p>}
      </div>
      <p>{post.body}</p>
      <h4>Comments</h4>
      {data.map((comment) => (
